let bars = [];
const def = "crimson", chng = "gold", finished = "mediumspringgreen", selected = "blueviolet";
...
function Disable_The_Input() {
    ...
    for (let i = 0; i < x.length; i++)
        x[i].disabled = true;
    return parseInt(1000 - document.getElementById("delay").value);
}
...
function generateBars(n = -1) {
    ...
    // n = n < 0 ? Math.floor(Math.random() * 25) + 1 : n;
    for (let i = 0; i < n; i++) {
        bars.push(`<div class="bar col" id="${i}" style="height:${Math.floor(Math.random() * 98) + 2}%"></div>`);
    }
